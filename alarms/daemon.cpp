...
sysrepo::ErrorCode mngrRPC(sysrepo::Session session, sysrepo::Event event, const libyang::DataNode input, libyang::DataNode output, sysrepo::Connection& dataConn, sysrepo::Session& dataSess) {
	...
	const bool cleared = severity == "cleared";
	...
	if (auto exists = activeAlarmExist(dataSess, id, qual, res, {prefixUnshelved, prefixShelved}); !exists && cleared) {
		return sysrepo::ErrorCode::Ok;
	}

	dataSess.setItem(prefix.c_str(), nullptr);
	dataSess.setItem((prefix + "/is-cleared").c_str(), cleared ? "true" : "false");

	if (!cleared) {
		dataSess.setItem((prefix + "/perceived-severity").c_str(), severity.c_str());
	}

	if (auto node = input.findPath("alarm-text")) {
		dataSess.setItem((prefix + "/alarm-text").c_str(), std::string(node.value().asTerm().valueStr()).c_str());
	}
