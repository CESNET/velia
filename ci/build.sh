...
if [[ -z "${ARTIFACT_URL}" ]]; then
    ...
    DEPSRCDIR=$(jq < ~/zuul-env.json -r "[.items[]? | select(.project.name == \"CzechLight/dependencies\")][-1]?.project.src_dir + \"\"")
    DEP_SUBMODULE_COMMIT=$(git ls-tree -l master ${HOME}/${DEPSRCDIR} | cut -d ' ' -f 3)
