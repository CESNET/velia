...
std::string generateNftConfig(velia::Log logger, const libyang::S_Data_Node& tree)
{
    ...
    const std::array skippedNodes{
        ...
        // Although we use a deviation for attachment-points, libyang still gives it to us when using tree_dfs. It isn't
        // possible for any value to be there.
        "/ietf-access-control-list:acls/attachment-points"sv,
        // ACL container
        ...
        // Logging is deviated, but still appears, see, attachment-points
        "/ietf-access-control-list:acls/acl/aces/ace/actions/logging"sv,
    };
    ...
    for (auto node : tree->tree_dfs()) {
        ...
        // Todo, the name mustn't have double quotes, handle that in deviations
        if (nodeSchemaPath == nodepaths::ace_comment) {
            ...
            if (action ==  "ietf-access-control-list:accept"sv) {
                ...
                // This should theoretically never happen, the only other value is "reject" and that one is disabled by
                // a deviation.
