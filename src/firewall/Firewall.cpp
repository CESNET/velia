    ...
    , m_log(spdlog::get("firewall"))
{
    ...
    if (auto mod = lyCtx->get_module("ietf-access-control-list", "2019-03-04"); !mod || !mod->implemented()) {
        throw std::runtime_error("ietf-access-control-list@2019-03-04 is not installed in sysrepo.");
    }
    if (auto mod = lyCtx->get_module("czechlight-firewall", "2021-01-25"); !mod || !mod->implemented()) {
