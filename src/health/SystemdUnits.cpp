    ...
    , m_proxyManager(sdbus::createProxy(connection, m_busName, managerObjectPath))
{
    ...
    // First, fetch all currently loaded units, register to their PropertiesChanged signal and create the alarm-inventory entry in a *single* edit
    ...
    // Ask for all the units once again. There could have been some that were registered between the previous ListUnits and the UnitNew registration
    units.clear();
    m_proxyManager->callMethod("ListUnits").onInterface(managerIface).storeResultsTo(units);
    for (const auto& unit : units) {
