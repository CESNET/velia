    ...
    , m_proxyManager(sdbus::createProxy(connection, m_busName, managerObjectPath))
{
    ...
    utils::alarms::createOrUpdateAlarmInventoryEntry(m_srSession, ALARM_ID, ALARM_INVENTORY_DESCRIPTION, unitNames, {ALARM_SEVERITY}, utils::alarms::WillClear::Yes);
...
void SystemdUnits::onUnitStateChange(const std::string& name, const UnitState& state)
{
    ...
    utils::alarms::createOrUpdateAlarm(m_srSession, ALARM_ID, name, alarmSeverity, "systemd unit state: (" + activeState + ", " + subState + ")");
}
