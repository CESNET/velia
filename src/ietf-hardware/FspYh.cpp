...
void FspYhPsu::createPower()
{
    ...
    try {
        m_eepromData.clear();
        auto data = velia::ietf_hardware::sysfs::ipmiFruEeprom(m_eeprom->sysfsEntry() / "eeprom");
        const auto& pi = data.productInfo;
        m_eepromData = {
            {xpathFor(m_namePrefix, "mfg-name"), pi.manufacturer},
            ...
            {xpathFor(m_namePrefix, "model-name"), fmt::format("{} ({})", pi.partNumber, pi.name)},
            {xpathFor(m_namePrefix, "hardware-rev"), pi.version},
            {xpathFor(m_namePrefix, "software-rev"), pi.fruFileId},
            {xpathFor(m_namePrefix, "serial-num"), pi.serialNumber},
            {xpathFor(m_namePrefix, "is-fru"), "true"},
        };
        if (pi.custom.size()) {
            ...
            m_eepromData[xpathFor(m_namePrefix, "firmware-rev")] = boost::algorithm::join(pi.custom, " ");
        }
    } catch (std::exception& e) {
        spdlog::get("hardware")->error("Cannot read PSU/PDU EEPROM: {}", e.what());
    }
