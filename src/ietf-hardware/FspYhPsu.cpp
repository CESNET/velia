...
void TransientI2C::bind() const
{
    ...
    if (!ofs.is_open()) {
        throw std::runtime_error("TransientI2C::bind(): can't open file");
    }
    ofs << m_driverName << " " << m_addressString;
    if (ofs.bad()) {
        throw std::runtime_error("TransientI2C::bind(): can't write file");
    }
}
void TransientI2C::unbind() const
{
    ...
    if (!ofs.is_open()) {
        throw std::runtime_error("TransientI2C::unbind(): can't open file");
    }
    ofs << m_addressString;
    if (ofs.bad()) {
        throw std::runtime_error("TransientI2C::unbind(): can't write file");
    }
}
