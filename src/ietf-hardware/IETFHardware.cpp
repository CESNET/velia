...
EepromWithUid::EepromWithUid(std::string componentName, std::optional<std::string> parent, const uint8_t bus, const uint8_t address)
    : DataReader(std::move(componentName), std::move(parent))
{
    namespace fs = std::filesystem;
    auto log = spdlog::get("hardware");

    auto dirname = fs::path{fmt::format("/sys/bus/i2c/devices/{}-{:04x}", bus, address)};
    ...
    try {
        if (!fs::is_regular_file(filename)) {
            // soft error: older clearfog boards don't have these EEPROMs populated
