 ...
 *
 * Note that a data reader can return any numbers of nodes and even multiple compoments.
 * For example, Fans data reader will create multiple data readers, one for each fan.
 */
...
struct _DataReader {
    ...
    _DataReader(std::string propertyPrefix, std::optional<std::string> parent);
};
...
struct StaticData : private _DataReader {
...
struct Fans : private _DataReader {
private:
...
struct SysfsTemperature : private _DataReader {
private:
    std::shared_ptr<sysfs::HWMon> m_hwmon;
...
struct EMMC : private _DataReader {
private:
