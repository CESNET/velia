...
using ThresholdsBySensorPath = std::map<std::string, Thresholds<uint64_t>>;
...
struct HardwareInfo {
    DataTree dataTree;
    std::map<std::string, ThresholdInfo> thresholdsInfo;
};
...
private:
    ...
    std::map<std::string, Watcher<uint64_t>> m_thresholdsWatchers;
};
...
private:
    ...
    Thresholds<uint64_t> m_thresholds;
