...
struct WithChecksum_directive : boost::spirit::x3::unary_parser<Subject, WithChecksum_directive<Subject>> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, attribute_type& attr) const
    {
        ...
        return std::accumulate(beginIter, begin, uint8_t{0}, [](uint8_t acc, uint8_t byte) { return acc + (int)byte; }) == 0;
    }
};
...
struct WithPadding_directive : boost::spirit::x3::unary_parser<Subject, WithPadding_directive<Subject>> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, attribute_type& attr) const
    {
        ...
        if (bytesReadTotal >= areaLength) {
            throw std::runtime_error("Padding: child parser already read " + std::to_string(bytesReadTotal) + " bytes, but the total length including padding is expected to be " + std::to_string(areaLength) + "");
        }
...
struct StringField : x3::parser<StringField> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, attribute_type& attr) const
    {
        ...
        if (type == Type::LanguageCode) {
            ...
            uint8_t typeLengthByte = (typeByte << 6) | length;
            throw std::runtime_error("Unimplemented type in type/length byte (" + std::to_string(typeByte) + ", byte=" + std::to_string(typeLengthByte) + ")");
        }
#endif
...
FRUInformationStorage parse(std::vector<uint8_t>::const_iterator begin, std::vector<uint8_t>::const_iterator end)
{
    ...
    if (!headerParser) {
        throw std::runtime_error("Failed to parse EEPROM data Common Header");
    }
    ...
    if (!productInfoArea) {
        throw std::runtime_error("Failed to parse EEPROM data Product Info Area");
    }
...
{
    const auto data = readFile(eepromPath);
    return parse(data.begin(), data.end());
}

FRUInformationStorage eepromData(const std::string& i2cBus, const std::string& i2cAddress)
{
    return eepromData(std::filesystem::path{"/sys/bus/i2c/devices"} / ("i2c-" + i2cBus) / (i2cBus + "-" + i2cAddress) / "eeprom");
}
}
