...
{
    std::vector<uint32_t> lifeTime = velia::utils::readFileBytes(rootDir / "life_time", 2);
    std::vector<uint32_t> eol = velia::utils::readFileBytes(rootDir / "pre_eol_info", 1);
...
std::map<std::string, std::string> EMMC::attributes() const
{
    ...
    std::for_each(ATTRIBUTE_FILES.begin(), ATTRIBUTE_FILES.end(), [&result, this](const std::string& filename) {
        result.insert(std::make_pair(filename, velia::utils::readFileString(m_root / filename)));
    });
    result.insert(std::make_pair("life_time", impl::processLifeTimeProperty(m_root)));
