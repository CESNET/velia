...
struct WithPadding_directive : boost::spirit::x3::unary_parser<Subject, WithPadding_directive<Subject, SavePositionTag>> {
    ...
    bool parse(It& begin, It end, Ctx const& ctx, RCtx& rctx, attribute_type& attr) const
    {
        ...
        size_t areaLength = 0;
        auto saveAreaLength = [&areaLength](auto& ctx) { areaLength = 8 * _attr(ctx); };

        It originalBegin;
        auto getSavedPosition = [&originalBegin](auto& ctx) { originalBegin = get<SavePositionTag>(ctx).get(); };

        const auto grammar = x3::eps[getSavedPosition] >> // fetch the start of the padded area
            x3::omit[x3::byte_[saveAreaLength]] >> // parse the length byte of the entire area and save the area length
        ...
        // skip padding bytes
        const size_t bytesReadTotal = std::distance(originalBegin, begin);
