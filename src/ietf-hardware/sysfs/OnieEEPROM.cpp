...
auto TLVEntry = x3::rule<struct TLVEntry, TLV>{"TLVEntry"} = //
    ...
    (typeCode(TLV::Type::DeviceVersion) >> x3::omit[byteWithValue(0x01)] >> x3::byte_) |
    (typeCode(TLV::Type::VendorExtension) >> blob);

auto TlvInfoString = x3::rule<struct TlvInfoString, std::string>{"TlvInfoString"} = //
    byteWithValue('T') >> // this literally spells "TlvInfo" in ASCII. The 8th byte is probably arbitrary?
    byteWithValue('l') >>
    byteWithValue('v') >>
    byteWithValue('I') >>
    byteWithValue('n') >>
