        ...
        while (!m_quit) {
            ...
            for (const auto& [leaf, value] : hwStateValues) {
                if (boost::ends_with(leaf, "/sensor-data/oper-status")) {
                    ...
                    if (value == "nonoperational" && prevValue != "nonoperational") {
                    ...
                    } else if (value == "ok" && prevValue && prevValue != "ok" /* don't invoke clear if we see this node for the first time */) {
                        auto componentXPath = extractComponentPrefix(leaf);
