    ...
    , m_quit(false)
{
    m_pollThread = std::thread([&]() {
        ...
        utils::alarms::createOrUpdateAlarmInventoryEntry(m_session, ALARM_THRESHOLD_CROSSING_LOW, "Sensor value is below the low threshold.", {}, {}, utils::alarms::WillClear::Yes);
        utils::alarms::createOrUpdateAlarmInventoryEntry(m_session, ALARM_THRESHOLD_CROSSING_HIGH, "Sensor value is above the high threshold.", {}, {}, utils::alarms::WillClear::Yes);
        utils::alarms::createOrUpdateAlarmInventoryEntry(m_session, ALARM_SENSOR_MISSING, "Sensor is missing.", {}, {}, utils::alarms::WillClear::Yes);
        utils::alarms::createOrUpdateAlarmInventoryEntry(m_session, ALARM_SENSOR_NONOPERATIONAL, "Sensor is flagged as nonoperational.", {}, {}, utils::alarms::WillClear::Yes);
        ...
        while (!m_quit) {
            ...
            if (!newSensors.empty()) {
                utils::alarms::addResourcesToAlarmInventoryEntry(m_session, ALARM_THRESHOLD_CROSSING_LOW, newSensors);
                utils::alarms::addResourcesToAlarmInventoryEntry(m_session, ALARM_THRESHOLD_CROSSING_HIGH, newSensors);
                utils::alarms::addResourcesToAlarmInventoryEntry(m_session, ALARM_SENSOR_MISSING, newSensors);
                utils::alarms::addResourcesToAlarmInventoryEntry(m_session, ALARM_SENSOR_NONOPERATIONAL, newSensors);
            }
            ...
            for (const auto& [alarm, resource, severity, text] : sideLoadedAlarms) {
                ...
                if (isActive && severity == "cleared") {
                    utils::alarms::createOrUpdateAlarm(m_session, alarm, resource, "cleared", text);
                    activeSideLoadedAlarms.erase({alarm, resource});
                } else if (!isActive && severity != "cleared") {
                    utils::alarms::createOrUpdateAlarm(m_session, alarm, resource, severity, text);
            ...
            for (const auto& [leaf, value] : hwStateValues) {
                if (boost::ends_with(leaf, "/sensor-data/oper-status")) {
                    ...
                    if (value == "nonoperational" && oldValue != "nonoperational") {
                        utils::alarms::createOrUpdateAlarm(m_session, ALARM_SENSOR_NONOPERATIONAL, extractComponentPrefix(leaf), ALARM_SENSOR_NONOPERATIONAL_SEVERITY, ALARM_SENSOR_NONOPERATIONAL_DESCRIPTION);
                        ...
                        utils::alarms::createOrUpdateAlarm(m_session, ALARM_SENSOR_NONOPERATIONAL, extractComponentPrefix(leaf), ALARM_CLEARED, ALARM_SENSOR_NONOPERATIONAL_DESCRIPTION);
                    }
                }
            }
            ...
            for (const auto& [sensorXPath, state] : thresholds) {
                ...
                if (state == State::NoValue) {
                    ...
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_SENSOR_MISSING, componentXPath, ALARM_MISSING_SEVERITY, ALARM_MISSING_DESCRIPTION);
                     ...
                     */
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_SENSOR_MISSING, componentXPath, ALARM_CLEARED, ALARM_MISSING_DESCRIPTION);
                }
                ...
                if (isThresholdCrossingLow(state)) {
                    ...
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_THRESHOLD_CROSSING_LOW, componentXPath, toYangAlarmSeverity(state), ALARM_THRESHOLD_CROSSING_LOW_DESCRIPTION);
                    ...
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_THRESHOLD_CROSSING_HIGH, componentXPath, toYangAlarmSeverity(state), ALARM_THRESHOLD_CROSSING_HIGH_DESCRIPTION);
                }
                ...
                if (!isThresholdCrossingLow(state) && isThresholdCrossingLow(prevState)) {
                    ...
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_THRESHOLD_CROSSING_LOW, componentXPath, ALARM_CLEARED, ALARM_THRESHOLD_CROSSING_LOW_DESCRIPTION);
                    ...
                    utils::alarms::createOrUpdateAlarm(m_session, ALARM_THRESHOLD_CROSSING_HIGH, componentXPath, ALARM_CLEARED, ALARM_THRESHOLD_CROSSING_HIGH_DESCRIPTION);
                }
