    ...
    , m_quit(false)
{
    m_pollThread = std::thread([&]() {
        ...
        std::set<std::pair<std::string, std::string>> sideloadedAlarmsResourcePairs;
        ...
        while (!m_quit) {
            ...
            for (const auto& [alarm, resource, severity, text] : sideLoadedAlarms) {
                ...
                if (!sideloadedAlarmsResourcePairs.contains({alarm, resource})) {
                    alarms::addResourcesToInventory(m_session, {{ALARM_SENSOR_MISSING, {resource}}});
                    sideloadedAlarmsResourcePairs.insert({alarm, resource});
                }
