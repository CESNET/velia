    ...
    , m_log(spdlog::get("system"))
{
    ...
    std::string raucPrimarySlot = rauc->Primary();
    auto raucPrimarySlotStatus = rauc->SlotStatus().at(raucPrimarySlot);

    std::map<std::string, std::string> opsSystemStateData {
        {IETF_SYSTEM_STATE_MODULE_PREFIX + "platform/os-name", std::get<std::string>(raucPrimarySlotStatus.at("bundle.compatible"))},
        {IETF_SYSTEM_STATE_MODULE_PREFIX + "platform/os-release", std::get<std::string>(raucPrimarySlotStatus.at("bundle.version"))},
    };

    sr_datastore_t oldDatastore;
    auto datastoreGuard = utils::make_unique_resource(
        [this, &oldDatastore]() {
            oldDatastore = m_srSession->session_get_ds();
