...
# Colorize every array element based on the severity
...
def formatAlarmType($type;$qualifier):
    if($type == "velia-alarms:sensor-low-value-alarm") then
        "\u2103 \u23f7" # ℃ ⏷ (degrees celsius + upwards triangle)
    elif($type == "velia-alarms:sensor-high-value-alarm") then
        "\u2103 \u23f6" # ℃ ⏶ (degrees celsius + downwards triangle)
...
def main:
  (["", "Resource", "Severity", "Detail", "Last raised", "Cleared"]),
  ( .
