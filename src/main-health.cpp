...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        std::vector<std::function<void(velia::health::State)>> outputs;
        if (const auto& appliance = args["--appliance"]) {
            spdlog::get("health")->debug("Initializing LED drivers");
            outputs.emplace_back(velia::health::createOutput(appliance.asString()));
        }

        velia::health::AlarmsOutputs alarms(srSessionAlarms, outputs);
