...
Options:
  ...
  --manager-log-level=<N>           Log level for manager [default: 2]
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // input configuration
        auto inputSystemdDbus = std::make_unique<velia::DbusSystemdInput>(manager, *dbusConnection);
        auto inputSemaphoreDbus = std::make_unique<velia::DbusSemaphoreInput>(manager, *dbusConnection, "bus.name", "obj.path", "Semaphore", "propIface");
