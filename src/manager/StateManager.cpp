...
/**
 * @brief Create new input socket associated to @p input
 * @return pointer to the input socket
 */
std::weak_ptr<StateManagerInput> StateManager::addInput(std::shared_ptr<AbstractInput> input)
{
...
/**
 * @brief Returns all the input sockets. So far only for testing purposes.
 */
...
/** @brief Notify that one of the input values changed. Recalculates the output */
void StateManager::notifyInputChanged()
{
    ...
    if (it != m_inputs.end() && (*it)->getInputValue() != State::UNINITIALIZED)
...
StateManagerInput::StateManagerInput(StateManager& manager, std::shared_ptr<AbstractInput> input)
    ...
    , m_inputValue(State::UNINITIALIZED)
{
}
