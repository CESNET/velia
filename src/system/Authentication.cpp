...
namespace {
std::string formatAuthorizedKeys(std::string_view authorized_keys_format, const std::string& username)
{
...
void writeKeys(std::string_view authorized_keys_format, const std::string& username, const std::vector<std::string>& keys)
{
    ...
    utils::safeWriteFile(formatAuthorizedKeys(authorized_keys_format, username), ss.str());
}
}
...
std::vector<std::string> listKeys(std::string_view authorized_keys_format, const std::string& username)
{
    ...
    std::ifstream ifs(formatAuthorizedKeys(authorized_keys_format, username));
    if (!ifs.is_open()) {
        return res;
    }
    std::string line;
