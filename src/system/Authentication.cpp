...
void safeWriteFile(const std::string& filename, const std::string_view& contents)
{
    auto throwErr = [&filename] (const auto& what) {
        spdlog::get("system")->critical("{} failed, errno {}", what, std::strerror(errno));
...
void writeKeys(std::string_view authorized_keys_format, const std::string& username, const std::vector<std::string>& keys)
{
    ...
    safeWriteFile(fmt::format(authorized_keys_format, username), ss.str());
}
}
