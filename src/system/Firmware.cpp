...
Firmware::Firmware(::sysrepo::Connection srConn, sdbus::IConnection& dbusConnectionSignals, sdbus::IConnection& dbusConnectionMethods)
    : m_rauc(std::make_shared<RAUC>(
        ...
        [this](int32_t perc, const std::string& msg) {
            ...
            session.sendNotification(*dataNode, sysrepo::Wait::Yes); // FIXME: should we wait or not?
        },
    ...
    , m_srSubscribeRPC()
{
    ...
    ::sysrepo::RpcActionCb cbRPC = [this](auto, auto, auto, auto input, auto, auto, auto) {
        auto lock = updateSlotStatus();

        try {
            auto source = std::string{input.asTerm().valueStr()};
            m_rauc->install(source);
        } catch (sdbus::Error& e) {
            m_log->warn("RAUC install error: '{}'", e.what());
            // TODO: implement errors in sysrepo::Session
    ...
    ::sysrepo::OperGetItemsCb cbOper = [this](auto session, auto, auto, auto, auto, auto, auto& parent) {
        std::map<std::string, std::string> data;
    ...
    m_srSubscribeOps = m_srSessionOps.onOperGetItems(
