...
namespace {
const auto czechlight_system_module = "czechlight-system";
const auto hostname_path = "/czechlight-system:hostname/hostname";
}
    ...
    , m_srSubscribe(std::make_shared<sysrepo::Subscribe>(m_srSession))
{
    ...
    utils::ensureModuleImplemented(m_srSession, czechlight_system_module, "2021-01-13");
    ...
    sysrepo::ModuleChangeCb cb = [] (
        ...
        auto data = session->get_changes_iter(hostname_path);


        while (auto change = session->get_change_tree_next(data)) {
            auto node = change->node();
            if (node->path() == hostname_path) {
    ...
    m_srSubscribe->module_change_subscribe(czechlight_system_module, cb, hostname_path, 0, SR_SUBSCR_DONE_ONLY | SR_SUBSCR_ENABLED);
}
