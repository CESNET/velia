...
bool sysrepoPathExists(std::shared_ptr<sysrepo::Session> session, const std::string& path, sr_datastore_t ds)
{
    auto oldDs = session->session_get_ds();

    session->session_switch_ds(ds);
    auto val = session->get_items(path.c_str());
    session->session_switch_ds(oldDs);
...
void IETFInterfaces::onLinkUpdate(rtnl_link* link, int action) {
        ...
        if (action == NL_ACT_DEL) {
            ...
            } else if (sysrepoPathExists(m_srSession, IETF_INTERFACES + "/interface[name='" + name + "']/phys-address", SR_DS_OPERATIONAL)) { // delete physical address only if the path is present in Sysrepo; Sysrepo throws if not present
