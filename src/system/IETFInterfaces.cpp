          ...
          [this](rtnl_route* addr, int action) { onRouteUpdate(addr, action); }))
{
    ...
    m_srSubscribe->module_change_subscribe(
        IETF_INTERFACES_MODULE_NAME.c_str(), [this](auto session, auto, auto, auto, auto) { return onSysrepoUpdate(session, m_configDirPersistent); }, "/ietf-interfaces:interfaces", 0, SR_SUBSCR_DONE_ONLY);
