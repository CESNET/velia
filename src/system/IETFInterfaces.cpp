...
void IETFInterfaces::onLinkUpdate(rtnl_link* link, int action)
{
    ...
    if (action == NL_ACT_DEL) {
        ...
        values[IETF_INTERFACES + "/interface[name='" + name + "']/type"] = std::filesystem::exists("/sys/class/net/"s + name + "/bridge") ? "iana-if-type:bridge" : arpTypeToString(rtnl_link_get_arptype(link), m_log);
