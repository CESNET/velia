...
void IETFSystem::initClock(const std::filesystem::path& procStat)
{
    ...
    auto bootTimestamp = std::chrono::utc_time<std::chrono::utc_clock::duration>(std::chrono::seconds(readBootTimestamp(procStat)));
...
void IETFSystem::initNTP(sdbus::IConnection& connection, const std::string& dbusNameTimesync, const std::string& dbusNameTimedate)
{
    ...
    sysrepo::OperGetCb operCb = [&connection, this, dbusNameTimesync, dbusNameTimedate](auto session, auto, auto, auto, auto, auto, auto& parent) {
        ...
        try {
            ...
            for (auto& s : runtimeServers) ntpServers.emplace_back(std::move(s), "runtime");
            for (auto& s : systemServers) ntpServers.emplace_back(std::move(s), "system");
            for (auto& s : linkServers) ntpServers.emplace_back(std::move(s), "link");

            if (ntpServers.empty()) {
                for (auto& s : proxy->getProperty("FallbackNTPServers").onInterface(DBUS_TIMESYNC1_MANAGER_INTERFACE).get<std::vector<std::string>>()) {
                    ntpServers.emplace_back(std::move(s), "fallback");
                }
            }
