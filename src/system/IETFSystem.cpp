...
std::string createResolvedConfig(const std::vector<std::string>& hosts)
{
    std::ostringstream oss;
    oss << "# Autogenerated by velia. Do not edit." << std::endl;
    oss << "[Resolve]" << std::endl;
    oss << "DNS=";
...
std::vector<std::string> getDNSServersFromChange(const sysrepo::Session session)
{
    std::vector<std::string> addresses;

    if (auto data = session.getData(IETF_SYSTEM_DNS_PATH)) {
        for (const auto& server : data->findXPath(IETF_SYSTEM_DNS_PATH + "/server/udp-and-tcp"s)) {
            ...
            spdlog::trace("Adding DNS server '{}' in {} config", address, session.activeDatastore() == sysrepo::Datastore::Running ? "running" : "startup");
...
void IETFSystem::initDNS(sdbus::IConnection& connection, const std::string& dbusName, const SystemdResolvedConfig& resolved)
{
    ...
    sysrepo::ModuleChangeCb dnsRunning = [resolved] (auto session, auto, auto, auto, auto, auto) {
        ...
        utils::safeWriteFile(resolved.runtimeDir / "resolved.conf", createResolvedConfig(getDNSServersFromChange(session)));
        resolved.reload();
        return sysrepo::ErrorCode::Ok;
    };

    sysrepo::ModuleChangeCb dnsStartup = [] (auto session, auto, auto, auto, auto, auto) {
        utils::safeWriteFile(BACKUP_ETC_RESOLVED_CONF_FILE, createResolvedConfig(getDNSServersFromChange(session)));
        return sysrepo::ErrorCode::Ok;
    };
