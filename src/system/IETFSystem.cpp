...
std::string zeroPaddedIndex(int i, unsigned width)
{
    return fmt::format("{:0>{}}", i, width);
}
...
std::string createSystemdDropinConfig(const std::string& section, const std::string& key, const std::vector<std::string>& hosts)
{
    ...
    oss << "# Autogenerated by velia. Do not edit.\n";
    oss << "[" << section << "]\n";
    oss << key << "=";
...
std::vector<std::string> getNTPOrDNSServersFromChange(const std::string& serverPath, const sysrepo::Session& session)
{
    ...
    if (auto data = session.getData(serverPath)) {
        ...
        for (const auto& server : data->findXPath(serverPath)) {
            const auto address = velia::utils::formatHostPort(server, "address", hasPortLeaf ? std::make_optional("port") : std::nullopt);
...
{
    return createSystemdDropinConfig("Time", "NTP", getNTPOrDNSServersFromChange(IETF_SYSTEM_NTP_PATH + "/server/udp"s, session));
}

std::string createResolvedConfig(const sysrepo::Session& session)
{
    return createSystemdDropinConfig("Resolve", "DNS", getNTPOrDNSServersFromChange(IETF_SYSTEM_DNS_PATH + "/server/udp-and-tcp"s, session));
}
...
void IETFSystem::initDNS(sdbus::IConnection& connection, const SystemdConfigData& resolve)
{
    ...
    sysrepo::OperGetCb dnsOper = [&connection, resolve] (auto session, auto, auto, auto, auto, auto, auto& parent) {
        ...
        auto padWidth = fmt::formatted_size("{}", servers.size());
        for (size_t i = 0; i < servers.size(); i++) {
            auto key = zeroPaddedIndex(i + 1, padWidth);
            values.emplace_back(IETF_SYSTEM_DNS_PATH + "/server[name='"s + key + "']/udp-and-tcp/address", servers[i].address);
            if (servers[i].port) {
                values.emplace_back(IETF_SYSTEM_DNS_PATH + "/server[name='"s + key + "']/udp-and-tcp/port", std::to_string(*servers[i].port));
            }
        }
...
void IETFSystem::initNTP(sdbus::IConnection& connection, const SystemdConfigData& timesync, const SystemdConfigData& timedate)
{
    ...
    sysrepo::OperGetCb ntpOper = [&connection, this, timesync, timedate](auto session, auto, auto, auto, auto, auto, auto& parent) {
        ...
        try {
            ...
            for (size_t i = 0; i < ntpServers.size(); i++) {
                auto name = zeroPaddedIndex(i + 1, padWidth) + "-" + ntpServers[i].second;
    ...
    sysrepo::ModuleChangeCb ntpRunning = [timesync] (auto session, auto, auto, auto, auto, auto) {
        std::filesystem::create_directories(*timesync.dropinDir);
        utils::safeWriteFile(*timesync.dropinDir / "timesyncd.conf", createTimesyncdConfig(session));
        if (timesync.reload) {
            timesync.reload();
        }
        return sysrepo::ErrorCode::Ok;
