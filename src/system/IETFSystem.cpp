...
std::string createSystemdDropinConfig(const std::string& section, const std::string& key, const std::vector<std::string>& hosts)
{
    std::ostringstream oss;
    oss << "# Autogenerated by velia. Do not edit." << std::endl;
    oss << "[" << section << "]" << std::endl;
    oss << key << "=";
    std::copy(hosts.begin(), hosts.end(), std::experimental::make_ostream_joiner(oss, " "));
    oss << '\n';
    return oss.str();
}

std::vector<std::string> getDNSOrNTPServersFromChange(const std::string& serverPath, const sysrepo::Session& session)
{
    std::vector<std::string> addresses;

    if (auto data = session.getData(serverPath)) {
        for (const auto& server : data->findXPath(serverPath)) {
            const auto addressNode = velia::utils::getUniqueSubtree(server, "address");
            std::string address = addressNode->asTerm().valueStr();

            if (auto node = velia::utils::getUniqueSubtree(server, "port")) {
                // resolved.conf: When IPv6 address is specified with a port number, then the address must be in the square brackets
                const auto isIpv6 = addressNode->asTerm().valueType().internalPluginId().find("ipv6") != std::string::npos;
                if (isIpv6) {
                    address = "[" + address + "]";
                }

                address += ":" + velia::utils::asString(node.value());
            }
...
std::string createTimesyncdConfig(const sysrepo::Session& session)
{
    ...
    if (auto data = session.getData(IETF_SYSTEM_NTP_PATH + "/enabled"s)) {
        if (auto enabledNode = data->findPath(IETF_SYSTEM_NTP_PATH + "/enabled"s); enabledNode->asTerm().valueStr() == "true") {
            hosts = getDNSOrNTPServersFromChange(IETF_SYSTEM_NTP_PATH + "/server/udp"s, session);
        }
    }
...
void IETFSystem::initDNS(sdbus::IConnection& connection, const std::string& dbusName, const SystemdConfigData& resolved)
{
    ...
    sysrepo::ModuleChangeCb dnsRunning = [resolved] (auto session, auto, auto, auto, auto, auto) {
        std::filesystem::create_directories(resolved.runtimeDir);
        utils::safeWriteFile(resolved.runtimeDir / "resolved.conf", createResolvedConfig(session));
        resolved.reload();
        return sysrepo::ErrorCode::Ok;
    };
...
/** @short NTP callbacks */
void IETFSystem::initNTP(sdbus::IConnection& connection, const std::string& dbusName, const SystemdConfigData& timesyncd)
{
    utils::ensureModuleImplemented(m_srSession, IETF_SYSTEM_MODULE_NAME, "2014-08-06", {{"ntp"}});
    sysrepo::OperGetCb ntpOper = [&connection, dbusName](auto session, auto, auto, auto, auto, auto, auto& parent) {
    ...
    sysrepo::ModuleChangeCb ntpRunning = [timesyncd] (auto session, auto, auto, auto, auto, auto) {
        std::filesystem::create_directories(timesyncd.runtimeDir);
        utils::safeWriteFile(timesyncd.runtimeDir / "timesyncd.conf", createTimesyncdConfig(session));
        timesyncd.reload();
        return sysrepo::ErrorCode::Ok;
    };

    sysrepo::ModuleChangeCb ntpStartup = [] (auto session, auto, auto, auto, auto, auto) {
        utils::safeWriteFile(BACKUP_ETC_SYSTEMD_TIMESYNCD_CONF_FILE, createTimesyncdConfig(session));
        return sysrepo::ErrorCode::Ok;
    };

    m_srSession.switchDatastore(sysrepo::Datastore::Running);
    m_srSubscribe->onModuleChange(IETF_SYSTEM_MODULE_NAME, ntpRunning, IETF_SYSTEM_NTP_PATH, 0, sysrepo::SubscribeOptions::DoneOnly | sysrepo::SubscribeOptions::Enabled);
    m_srSession.switchDatastore(sysrepo::Datastore::Startup);
    m_srSubscribe->onModuleChange(IETF_SYSTEM_MODULE_NAME, ntpStartup, IETF_SYSTEM_NTP_PATH, 0, sysrepo::SubscribeOptions::DoneOnly);
    m_srSession.switchDatastore(sysrepo::Datastore::Operational);
    m_srSubscribe->onOperGet(IETF_SYSTEM_MODULE_NAME, ntpOper, IETF_SYSTEM_NTP_PATH);
}
    ...
    , m_log(spdlog::get("system"))
{
    ...
    initDummies();
    initClock(procStat);
    initDNS(connection, dbusName, resolved);
    initNTP(connection, dbusName, timesyncd);
}
}
