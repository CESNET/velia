...
{
    sysrepo::RpcActionCb cb = [this](auto, auto, auto, auto, auto, auto, auto) {
            try {
                velia::utils::execAndWait(m_log, SYSTEMCTL_EXECUTABLE, {"reboot"}, "", {});
            } catch(const std::runtime_error& e) {
                // FIXME: add errors to sysrepo
...
void IETFSystem::initHostname()
{
    ...
    sysrepo::OperGetItemsCb hostNameCbOperational = [] (auto, auto, auto, auto, auto, auto, auto& parent) {
    ...
    m_srSubscribe->onOperGetItems(IETF_SYSTEM_MODULE_NAME.c_str(), hostNameCbOperational, IETF_SYSTEM_HOSTNAME_PATH);
}
...
{
    sysrepo::OperGetItemsCb cb = [] (auto, auto, auto, auto, auto, auto, auto& parent) {
    ...
    m_srSubscribe->onOperGetItems(IETF_SYSTEM_MODULE_NAME.c_str(), cb, IETF_SYSTEM_STATE_CLOCK_PATH, sysrepo::SubscribeOptions::OperMerge);
}

/** @short DNS resolver callbacks */
void IETFSystem::initDNS(sdbus::IConnection& connection, const std::string& dbusName) {
    sysrepo::OperGetItemsCb dnsOper = [&connection, dbusName] (auto session, auto, auto, auto, auto, auto, auto& parent) {
    ...
    m_srSubscribe->onOperGetItems(IETF_SYSTEM_MODULE_NAME.c_str(), dnsOper, IETF_SYSTEM_DNS_PATH);
}
