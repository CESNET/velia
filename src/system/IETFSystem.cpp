...
std::string getHostnameFromChange(const std::shared_ptr<sysrepo::Session> session)
{
    ...
    throw std::runtime_error("Hostname changed, but no change was found.");
}
}
    ...
    , m_log(spdlog::get("system"))
{
    ...
    sysrepo::ModuleChangeCb hostNameCbRunning = [this] (
        ...
        velia::utils::execAndWait(m_log, HOSTNAMECTL_EXECUTABLE, {"set-hostname", getHostnameFromChange(session)}, "");
        return SR_ERR_OK;
    };
    ...
    sysrepo::ModuleChangeCb hostNameCbStartup = [] (
        ...
        utils::safeWriteFile(BACKUP_ETC_HOSTNAME_FILE, getHostnameFromChange(session));
        return SR_ERR_OK;
    };

    m_srSubscribeRunning->module_change_subscribe(IETF_SYSTEM_MODULE_NAME.c_str(), hostNameCbRunning, IETF_SYSTEM_HOSTNAME_PATH, 0, SR_SUBSCR_DONE_ONLY | SR_SUBSCR_ENABLED);
    m_srSubscribeStartup->module_change_subscribe(IETF_SYSTEM_MODULE_NAME.c_str(), hostNameCbStartup, IETF_SYSTEM_HOSTNAME_PATH, 0, SR_SUBSCR_DONE_ONLY | SR_SUBSCR_ENABLED);
}
}
