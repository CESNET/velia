...
namespace {
sysrepo::ErrorCode configureJournalUpload(velia::Log log, sysrepo::Session session, const std::filesystem::path& envFile, const velia::system::JournalUpload::RestartCb& restartCb)
{
    std::optional<std::string> url;

    if (auto data = session.getData(UPLOAD_URL_LEAF)) {
...
sysrepo::Subscription journalUploadSubscription(velia::Log log, sysrepo::Session session, const std::filesystem::path& envFile, const velia::system::JournalUpload::RestartCb& restartCb)
{
    auto sub = session.onModuleChange(
        ...
        [log, envFile, restartCb](auto session, auto, auto, auto, auto, auto) {
            return configureJournalUpload(log, session, envFile, restartCb);
        },
        std::nullopt,
        0,
     ...
     */
    configureJournalUpload(log, session, envFile, restartCb);
