    ...
    , m_thrRunning(true)
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator(sysfsLeds)) {
        ...
        m_leds.push_back(sysfsLeds / entry.path().filename());
    }
    ...
    while (m_thrRunning) {
        ...
        for (const auto& ledDirectory : m_leds) {
            ...
            try {
                ...
                m_log->trace("Found LED '{}' with brightness of {} % (brightness {} out of {})", std::string(deviceName), percent, brightness, max_brightness);
