...
{
    // we don't want to overwrite installed installNotifier when installation in progress
    if (std::string operation = m_dbusObjectProxy->getProperty("Operation").onInterface(INTERFACE); operation == "idle") {
        {
            std::lock_guard<std::mutex> lck(m_mtx);
            m_installNotifier = std::move(installNotifier);
        }
        m_dbusObjectProxy->callMethod("InstallBundle").onInterface(INTERFACE).withArguments(source); // this will result in sdbus::Error if another operation in progress
    } else {
        throw std::logic_error("Another RAUC operation in progress");
    }
}
