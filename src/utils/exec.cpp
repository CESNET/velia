...
void velia::utils::execAndWait(velia::Log logger, const std::string& program, std::initializer_list<std::string> args, const std::optional<std::string>& std_in)
{
    ...
    bp::child c(bp::search_path(program), boost::process::args=std::move(args), bp::std_in < stdinPipe, bp::std_err > stderrStream);

    if (std_in) {
        stdinPipe.write(std_in->c_str(), std_in->size());
        stdinPipe.close();
    }
