...
using HardwareStateDataTree = std::map<std::string, std::string>;
...
public:
    using DataReader = std::function<HardwareStateDataTree()>;
...
struct Component {
    ...
    HardwareStateDataTree m_staticData;
...
struct Roadm : private Component { // FIXME: decide how to change this. It definitely wont be in production. Maybe redesign into a "generic static-data-only node"?
    ...
    HardwareStateDataTree operator()() const;
};
