...
void valuesToYang(const std::map<std::string, std::string>& values, std::shared_ptr<::sysrepo::Session> session, std::shared_ptr<libyang::Data_Node>& parent)
{
    for (const auto& [propertyName, value] : values) {
        ...
        spdlog::get("hardware")->debug("propertyName: {}, value: {}", propertyName, value.c_str());
        spdlog::get("hardware")->trace("  processing {}", valuePath);
...
{
    spdlog::get("hardware")->debug("operational data callback: XPath {} req {} orig-XPath {}", xpath, request_id, request_xpath);
