...
TEST_CASE("factory")
{
    ...
    SECTION("bidi")
    {
        ...
        SECTION("clearfog eeprom")
        {
            ...
            // Even if the code cannot read from the EEPROM, it still "knows" that we're on a ClearFog
            ...
            SECTION("empty")
            {
                // The on-board EEPROM (24AA025UID) stores its own S/N as a byte sequence at a particular offset.
                // There's no checksum and therefore no way of knowing whether the data are "valid".
                // The same EEPROM also stores the ONIE-formatted data at the beginning of its address space.
                // This test tries to verify what happens if the EEPROM was never programmed by SolidRun. In that case,
                // the memory reads as a sequence of 0xFF bytes. On a real board, the S/N "should" be still present
                // because that one is burned at the chip factory, and it's physically read-only, but the test helper
                // makeBlankEeprom() doesn't do that for obvious reasons.
