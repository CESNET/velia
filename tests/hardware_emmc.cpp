...
TEST_CASE("EMMC driver")
{
    ...
    SECTION("Test correct structure")
    {
        ...
        fs::copy(sourceDir, fakeRoot, fs::copy_options::recursive);
        auto emmcAttrs = velia::hardware::sysfs::EMMC(fakeRoot);
        REQUIRE(emmcAttrs.attributes() == expected);
    }

    SECTION("Test emmc (<5) / device3")
    {
        fs::copy(CMAKE_CURRENT_SOURCE_DIR + "/tests/sysfs/emmc/device3"s, fakeRoot, fs::copy_options::recursive);

        // health reporting missing (emmc < 5). When one file is missing, the attributes method invocation throws
        REQUIRE_THROWS_AS(velia::hardware::sysfs::EMMC(fakeRoot).attributes(), velia::hardware::sysfs::FileDoesNotExist);
    }
}
