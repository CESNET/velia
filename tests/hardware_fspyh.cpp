...
TEST_CASE("FspYhPsu")
{
    ...
    REQUIRE_CALL(*fakeEEPROM, unbind_mock()).LR_WITH(counter == 3).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakePMBus, bind_mock()).LR_WITH(counter == 4).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakeEEPROM, bind_mock()).LR_WITH(counter == 4).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakePMBus, unbind_mock()).LR_WITH(counter == 5).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakeEEPROM, unbind_mock()).LR_WITH(counter == 5).IN_SEQUENCE(seq1);
    ...
    for (auto i : {0, 1, 2, 3, 4, 5}) {
        ...
        case 4:
            ...
            // a PSU that's no longer there and the watcher thread didn't unbind it yet. The EEPROM also hasn't been unbound yet.
            fakePMBus->removeHwmonFile("temp1_input");
            expected = expectedDisabled;
            expected["/ietf-hardware:hardware/component[name='ne:psu']/mfg-name"] = "3Y POWER";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/model-name"] = "YH-5151E (URP1X151AH)";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/is-fru"] = "true";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/hardware-rev"] = "B01R";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/firmware-rev"] = "A14";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/software-rev"] = "P2J700A01";
            expected["/ietf-hardware:hardware/component[name='ne:psu']/serial-num"] = "SA140T302044001013";
            expectedThresholdsKeys.clear();
            expectedAlarms = {alarmUnplugged};
            break;
        case 5:
