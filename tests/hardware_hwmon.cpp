...
TEST_CASE("HWMon class")
{
    ...
    SECTION("Test hwmon/device2")
    {
        ...
        expected = {
            ...
            {"temp1_input", -34'000},
            {"temp1_max", 80'000},
            /* -9223372036854775808LL (min value for int64_t) can not be input in the program as a literal value because it consists
             * of two tokens: "-", and "9223372036854775808LL". The second token is not representable as a int64_t value because it
             * overflows int64_t (by one). Therefore the compiler reports error: "integer constant is so large that it is unsigned".
             * See https://gcc.gnu.org/bugzilla/show_bug.cgi?id=52661
             *
             * Therefore, you can not input an (long long) integer literal for minimal value of int64_t. You have to work around and
             * - either input the second minimal value and subtract 1
             * - or you can use std::numeric_limits<int64_t>::min().
             * But I wanted to make clear that *this* is the contents. Not some symbolic name.
             */
