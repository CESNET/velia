...
TEST_CASE("HardwareState")
{
    ...
    SECTION("Test with sdn-roadm-line configuration")
    {
        std::map<std::string, std::string> textResults;
        ...
        hwState->registerComponent(velia::hardware::callback::Roadm("ne", ""));
        hwState->registerComponent(velia::hardware::callback::Controller("ne:ctrl", "ne"));
        hwState->registerComponent(velia::hardware::callback::Fans("ne:fans", "ne", fans, 4));
        ...
        SECTION("test components")
        {
            ...
            srSess->session_switch_ds(SR_DS_OPERATIONAL);
            REQUIRE(dataFromSysrepo(srSess, modulePrefix + "/component") == textResults);
            srSess->session_switch_ds(SR_DS_RUNNING);
        }
