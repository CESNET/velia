...
public:
    ...
    MAKE_CONST_MOCK0(isPresent, bool());
    ...
    void bind() const override
    {
        ...
        for (const auto& filename : {"name", "temp1_input"} )
        {
...
TEST_CASE("FspYhPsu")
{
    ...
    REQUIRE_CALL(*fakeI2c, isPresent()).RETURN(false).IN_SEQUENCE(seq1);

    auto psu = std::make_shared<velia::ietf_hardware::FspYhPsu>(fakeHwmonRoot, "psu", fakeI2c);

    // Then, the device appears
    REQUIRE_CALL(*fakeI2c, isPresent()).RETURN(true).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakeI2c, bind_mock()).IN_SEQUENCE(seq1);
    std::this_thread::sleep_for(std::chrono::seconds(4));
    // 4 seconds have passed => two calls to isPresent()

    // Then, the device disappears again
    REQUIRE_CALL(*fakeI2c, isPresent()).RETURN(false).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakeI2c, unbind_mock()).IN_SEQUENCE(seq1);
    std::this_thread::sleep_for(std::chrono::seconds(3));
    // 7 seconds have passed => three calls to isPresent()

    // Then, it appears again
    REQUIRE_CALL(*fakeI2c, isPresent()).RETURN(true).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*fakeI2c, bind_mock()).IN_SEQUENCE(seq1);
    std::this_thread::sleep_for(std::chrono::seconds(3));
    // 10 seconds have passed => four calls to isPresent()

    // Here I simulate read failure by deleting the hwmon directory and try to read values. This happens when the user
    // wants data from a PSU that's no longer there and the watcher thread didn't unbind it yet.
    removeDirectoryTreeIfExists(fakeHwmonRoot);

    REQUIRE(psu->readValues() == velia::ietf_hardware::DataTree{});

    // After that, the device disappears again.
    REQUIRE_CALL(*fakeI2c, isPresent()).RETURN(false).IN_SEQUENCE(seq1);
    std::this_thread::sleep_for(std::chrono::seconds(3));
    // 13 seconds have passed -> five calls to isPresent()
