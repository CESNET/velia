...
#define THRESHOLD(RESOURCE, STATE) {"/ietf-hardware:hardware/component[name='" RESOURCE "']/sensor-data/value", STATE}
...
TEST_CASE("HardwareState")
{
...

    {
        ...
        REQUIRE(alarms == std::map<std::string, velia::ietf_hardware::State>{
                    THRESHOLD("ne:ctrl:current", State::Disabled),
                    THRESHOLD("ne:ctrl:power", State::Disabled),
                    THRESHOLD("ne:ctrl:temperature-cpu", State::Disabled),
                    THRESHOLD("ne:ctrl:voltage-in", State::Disabled),
                    THRESHOLD("ne:ctrl:voltage-out", State::Disabled),
                    THRESHOLD("ne:ctrl:emmc:lifetime", State::WarningLow),
                    THRESHOLD("ne:fans:fan1:rpm", State::Normal),
                    THRESHOLD("ne:fans:fan2:rpm", State::Normal),
                    THRESHOLD("ne:fans:fan3:rpm", State::Normal),
                    THRESHOLD("ne:fans:fan4:rpm", State::Normal),
                    THRESHOLD("ne:psu:child", State::WarningHigh),
                });
    }
    ...
    expected["/ietf-hardware:hardware/component[name='ne:fans:fan2:rpm']/sensor-data/value"] = "11500";
    {
        ...
        REQUIRE(data == expected);
        REQUIRE(alarms == std::map<std::string, velia::ietf_hardware::State>{
                    THRESHOLD("ne:fans:fan2:rpm", State::WarningHigh),
                });
    }
...

    {
        ...
        REQUIRE(data == expected);
        REQUIRE(alarms == std::map<std::string, velia::ietf_hardware::State>{
                    THRESHOLD("ne:fans:fan2:rpm", State::CriticalHigh),
                    THRESHOLD("ne:psu:child", State::NoValue),
                });
    }
