...
TEST_CASE("HardwareState")
{
    ...
    std::vector<uint64_t> fanValues = {253, 0, 1280, 666};
    ...
    ietfHardware->registerDataReader(Fans("ne:fans", "ne", fans, 4, Thresholds<uint64_t>{std::nullopt, std::nullopt, OneThreshold<uint64_t>{10000, 2000}, OneThreshold<uint64_t>{15000, 2000}}));
    ietfHardware->registerDataReader(SysfsValue<SensorType::Temperature>("ne:ctrl:temperature-cpu", "ne:ctrl", sysfsTempCpu, 1, Thresholds<uint64_t>{OneThreshold<uint64_t>{5000, 1000}, OneThreshold<uint64_t>{10000, 1000}, OneThreshold<uint64_t>{50000, 1000}, OneThreshold<uint64_t>{60000, 1000}}));
    ...
    ietfHardware->registerDataReader(EMMC("ne:ctrl:emmc", "ne:ctrl", emmc, Thresholds<uint64_t>{OneThreshold<uint64_t>{10, 0}, OneThreshold<uint64_t>{20, 0}, std::nullopt, std::nullopt}));
