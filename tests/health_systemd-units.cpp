...
#define REQUIRE_ALARM_INVENTORY_UNIT(UNIT) REQUIRE_DATASTORE_CHANGE(alarmInventoryChanges, (ValueChanges{ \
        ...
        })).IN_SEQUENCE(seq1).ALARM_INVENTORY_INSERT(alarmInventory, "velia-alarms:systemd-unit-failure", "", UNIT, std::nullopt)
...
TEST_CASE("systemd unit state monitoring (alarms)")
{
    ...
    REQUIRE_DATASTORE_CHANGE(alarmInventoryChanges,
                             ...
                             })).IN_SEQUENCE(seq1).ALARM_INVENTORY_INSERT(alarmInventory, "velia-alarms:systemd-unit-failure", "", std::nullopt, "critical");
