...
TEST_CASE("systemd unit state monitoring (alarms)")
{
    ...
    EXPECT_ALARM_RPC("unit2.service", "major", "systemd unit state: (activating, auto-restart)");
    server.createUnit(*serverConnection, "unit2.service", "/org/freedesktop/systemd1/unit/unit2", "activating", "auto-restart");
    EXPECT_ALARM_RPC("unit3.service", "major", "systemd unit state: (failed, failed)");
    ...
    EXPECT_ALARM_RPC("unit4.service", "major", "systemd unit state: (failed, failed)");
