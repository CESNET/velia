...
TEST_CASE("Test semaphore input")
{
    ...
    REQUIRE_CALL(*mx, registerInput(ANY(void*), State::OK)).LR_SIDE_EFFECT(mx->updateState(_1, _2)).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*mx, updateState(ANY(void*), State::OK)).IN_SEQUENCE(seq1);
    REQUIRE_CALL(*mx, updateState(ANY(void*), State::ERROR)).IN_SEQUENCE(seq1);
    auto i1 = std::make_shared<cla::DbusSemaphoreInput>(mx, *clientConnection, serverConnection->getUniqueName(), dbusObj, dbusProp, dbusPropIface);
