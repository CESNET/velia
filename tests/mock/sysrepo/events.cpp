    ...
    : m_sub{session.onNotification(
                ...
                [this, xpath](sysrepo::Session, uint32_t, const sysrepo::NotificationType type, const std::optional<libyang::DataNode> tree, const sysrepo::NotificationTimeStamp) {
                    ...
                    for (const auto& it : tree->findPath(xpath)->childrenDfs()) {
                        if (it.path().find(xpath) != 0) {
                            throw std::logic_error{"NotificationWatcher: Got a strange notification for XPath " + xpath + ": data has XPath " + it.path()};
                        }
                        if (!it.isTerm()) {
                            continue;
                        }
                        data[it.path().substr(xpath.size() + 1)] = std::visit(libyang::ValuePrinter{}, it.asTerm().value());
                    }
                    notified(data);
                },
                xpath)}
