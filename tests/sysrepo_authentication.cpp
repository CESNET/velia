...
TEST_CASE("Authentication")
{
    ...
    SECTION("RPCs/actions")
    {
        ...
        SECTION("add key")
        {
            ...
            {
                expectation = NAMED_REQUIRE_CALL(mock, addKey(authorized_keys_format, "root", "invalid")).THROW(std::runtime_error("Invalid key."));;
                rpcPath = "/czechlight-system:authentication/users[name='root']/add-authorized-key";
        ...
        SECTION("remove key")
        {
            ...
            {
                expectation = NAMED_REQUIRE_CALL(mock, removeKey(authorized_keys_format, "root", 0)).THROW(std::runtime_error("Can't remove last key."));;
...
TEST_CASE("implementations")
{
    SECTION("key manipulation")
    {
        ...
        SECTION("listKeys")
        {
            ...
            SECTION("one-key")
            {
                ...
                result = velia::system::NAME_THIS::listKeys(authorized_keys_format, "one-key");
            }

            SECTION("two-key")
            {
                expected = {"ssh-rsa KEY_PLACEHOLDER ahoj@hostname", "ssh-rsa KEY_PLACEHOLDER another_key"};
                result = velia::system::NAME_THIS::listKeys(authorized_keys_format, "two-key");
            }

            SECTION("spaces")
            {
                expected = {"ssh-rsa KEY_PLACEHOLDER ahoj@hostname", "    ssh-rsa KEY_PLACEHOLDER another_key"};
                result = velia::system::NAME_THIS::listKeys(authorized_keys_format, "spaces");
            }

            SECTION("empty")
            {
                result = velia::system::NAME_THIS::listKeys(authorized_keys_format, "empty");
            }

            SECTION("key file doesn't exist")
            {
                result = velia::system::NAME_THIS::listKeys(authorized_keys_format, "no-key-file");
            }
        ...
        SECTION("add remove")
        {
            ...
            SECTION("addKey")
            {
                ...
                {
                    expectedContents = "ssh-rsa KEY_PLACEHOLDER ahoj@hostname\n";
                    REQUIRE_THROWS(velia::system::NAME_THIS::addKey(authorized_keys_format, "one-key", "invalid key"));
                    fileToCheck = keyDirectory + "one-key";
                }
            }
            ...
            SECTION("removeKey")
            {
                ...
                {
                    velia::system::NAME_THIS::removeKey(authorized_keys_format, "two-key", 0);
                ...
                {
                    REQUIRE_THROWS(velia::system::NAME_THIS::removeKey(authorized_keys_format, "one-key", 0));
                    expectedContents = "ssh-rsa KEY_PLACEHOLDER ahoj@hostname\n";
                    fileToCheck = keyDirectory + "one-key";
                }
            }
    ...
    SECTION("listUsers")
    {
        ...
        auto users = velia::system::NAME_THIS::listUsers("");
