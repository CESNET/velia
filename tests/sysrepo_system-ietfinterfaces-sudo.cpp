...
TEST_CASE("Test ietf-interfaces and ietf-routing")
{
    ...
    SECTION("Add a bridge")
    {
        ...
        iproute2_exec_and_wait(WAIT, "link", "set", "dev", IFACE, "up");
        iproute2_exec_and_wait(8 * WAIT /* it seems netlink thread is processing the changes rather slowly, let's wait a bit longer */, "-l", "0", "addr", "flush", "dev", IFACE); // sometimes, addresses are preserved even when enslaved
