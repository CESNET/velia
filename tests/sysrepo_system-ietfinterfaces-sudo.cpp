...
TEST_CASE("Test ietf-interfaces and ietf-routing")
{
    ...
    SECTION("Change physical address")
    {
        ...
        std::map<std::string, std::string> exp = initialExpected;
    ...
    SECTION("IPv6 LL gained when device up")
    {
        ...
        std::this_thread::sleep_for(2 * WAIT); // FIXME: why this needs more wait time? Usually fails without more time.
        ...
        {
            std::map<std::string, std::string> exp = initialExpected;
            exp.erase("/ietf-ip:ipv6/address[ip='::ffff:192.0.2.1']");
            exp.erase("/ietf-ip:ipv6/address[ip='::ffff:192.0.2.1']/ip");
            exp.erase("/ietf-ip:ipv6/address[ip='::ffff:192.0.2.1']/prefix-length");
