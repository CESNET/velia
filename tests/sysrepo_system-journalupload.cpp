...
TEST_CASE("Sysrepo czechlight-system:syslog")
{
    ...
    // ipv6
    EXPECT_RESTART_UNIT;
    client.setItem("/czechlight-system:journal-upload/host", "::1");
    client.applyChanges();
    waitForCompletionAndBitMore(seq1);
    REQUIRE(std::filesystem::exists(fakeEnvFile));
    REQUIRE(velia::utils::readFileToString(fakeEnvFile) == "DESTINATION=https://[::1]:19532\n");
    ...
    waitForCompletionAndBitMore(seq1);
    REQUIRE(std::filesystem::exists(fakeEnvFile));
    REQUIRE(velia::utils::readFileToString(fakeEnvFile) == "DESTINATION=https://[2001:db8:1::ab9:c0a8:102]:19532\n");
