...
TEST_CASE("Sysrepo opsdata callback")
{
    ...
    SECTION("Neighbors")
    {
      ...
      "Neighbors": [{
        ...
        auto lldp = velia::system::LLDPSysrepo(srSess, "/etc/machine-id", std::make_shared<velia::system::LLDPDataProvider>([&]() { return json; }));
    ...
    SECTION("Local data")
    {
        REQUIRE_THROWS_WITH_AS(velia::system::LLDPSysrepo(srSess, CMAKE_CURRENT_SOURCE_DIR "/tests/system/machine-id.doesnotexist", std::make_shared<velia::system::LLDPDataProvider>([&]() { return json; })),
                               ("File '"s + CMAKE_CURRENT_SOURCE_DIR "/tests/system/machine-id.doesnotexist' does not exist.").c_str(),
                               std::invalid_argument);

        auto lldp = velia::system::LLDPSysrepo(srSess, CMAKE_CURRENT_SOURCE_DIR "/tests/system/machine-id", std::make_shared<velia::system::LLDPDataProvider>([&]() { return json; }));
