...
TEST_CASE("System stuff in Sysrepo")
{
    ...
    SECTION("Test system-state")
    {
        ...
        SECTION("Valid data")
        {
            ...
            auto sysrepo = std::make_shared<velia::system::Sysrepo>(srSess, file, rauc);
            REQUIRE(dataFromSysrepo(client, modulePrefix, SR_DS_OPERATIONAL) == expected);
        }

        SECTION("Invalid data (missing VERSION and NAME keys)")
        {
            REQUIRE_THROWS_AS(std::make_shared<velia::system::Sysrepo>(srSess, CMAKE_CURRENT_SOURCE_DIR "/tests/system/missing-keys", rauc), std::out_of_range);
        }
    }
    ...
    {
        TEST_SYSREPO_INIT_CLIENT;
        auto sysrepo = std::make_shared<velia::system::Sysrepo>(srSess, CMAKE_CURRENT_SOURCE_DIR "/tests/system/os-release", rauc);

        libyang::S_Context ctx = clientConn->get_context();
        libyang::S_Module mod = ctx->get_module("/czechlight-system");

        auto input = std::make_shared<libyang::Data_Node>(ctx, "/czechlight-system:rauc-install", nullptr, LYD_ANYDATA_CONSTSTRING, 0);
        std::make_shared<libyang::Data_Node>(libyang::Data_Node(input, mod, "source", "/path/to/source/update.raucb"));
        ...
        auto res = client->rpc_send(input);
        REQUIRE(res->path() == "/czechlight-system:rauc-install");
        REQUIRE(res->child() == nullptr); // the RPC returns nothing
