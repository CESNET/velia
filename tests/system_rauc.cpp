...
TEST_CASE("Fetch RAUC data over DBus")
{
    ...
    SECTION("Install bundle")
    {
        ...
        {
            server.installBundleBehaviour(DBusRAUCServer::InstallBehaviour::OK);

            // We don't know how much the first install does
            FAKE_RAUC_OPERATION("installing");
            FAKE_RAUC_PROGRESS(0, "Installing").TIMES(AT_MOST(2));
            FAKE_RAUC_PROGRESS(0, "Determining slot states").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(20, "Determining slot states done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(20, "Checking bundle").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(20, "Veryfing signature").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(40, "Veryfing signature done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(40, "Checking bundle done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(40, "Loading manifest file").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(60, "Loading manifest file done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(60, "Determining target install group").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(80, "Determining target install group done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(80, "Updating slots").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(80, "Checking slot rootfs.0").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(85, "Checking slot rootfs.0 done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(85, "Copying image to rootfs.0").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(90, "Copying image to rootfs.0 done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(90, "Checking slot cfg.0").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(95, "Checking slot cfg.0 done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(95, "Copying image to cfg.0").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(100, "Copying image to cfg.0 done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(100, "Updating slots done.").TIMES(AT_MOST(1));
            FAKE_RAUC_PROGRESS(100, "Installing done.").TIMES(AT_MOST(1));
            FAKE_RAUC_OPERATION("idle").TIMES(AT_MOST(1));

            rauc->install("/path/to/bundle");
            REQUIRE_THROWS_AS(rauc->install("/path/to/bundle"), velia::system::RAUC::OperationInProgress);
            REQUIRE_THROWS_AS(rauc->slotStatus(), velia::system::RAUC::OperationInProgress);
            REQUIRE_THROWS_AS(rauc->primarySlot(), velia::system::RAUC::OperationInProgress);
